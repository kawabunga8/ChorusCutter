...
class WaveformWidget(FigureCanvasQTAgg):
    ...
    def load(
        ...
        y_ds = y[::step]
        times = np.linspace(0.0, duration, len(y_ds))

        self._ax.fill_between(times, y_ds, -y_ds,
                              color="#3d7ef0", alpha=0.35, linewidth=0)
        self._ax.plot(times,  y_ds, color="#5b9cf6", linewidth=0.7, alpha=0.9)
        self._ax.plot(times, -y_ds, color="#5b9cf6", linewidth=0.7, alpha=0.9)
